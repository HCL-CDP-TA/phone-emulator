...
// Helper to broadcast email to all connections for a phone number
export function broadcastEmailToPhone(
  phoneNumber: string,
  from: string,
  ...
  if (!connections || connections.size === 0) {
    console.log(`[Email SSE] No active connections for ${phoneNumber}`)
    return false
  }
  ...
  console.log(`[Email SSE] Broadcasting to ${connections.size} connection(s) for ${phoneNumber}`)
  ...
  for (const controller of connections) {
    try {
      ...
      console.error("[Email SSE] Failed to send to connection (dead connection):", error)
  ...
  console.log(`[Email SSE] Delivered to ${successCount}/${successCount + deadConnections.length} connections`)
...
export async function GET(request: NextRequest) {
  ...
  console.log(`[Email SSE] New connection for ${phoneNumber}`)
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      console.log(`[Email SSE] Active connections for ${phoneNumber}: ${activeConnections.get(phoneNumber)!.size}`)
      ...
      const cleanup = () => {
        ...
        if (connections) {
          ...
          console.log(`[Email SSE] Connection closed for ${phoneNumber}. Remaining: ${connections.size}`)
        }
      }
      ...
      const keepAliveInterval = setInterval(() => {
        try {
          ...
          console.log("[Email SSE] Keep-alive failed, connection closed")
